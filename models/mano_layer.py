...
class MANOHandModel:
        ...
        if left_mano_output.joints.shape[1] != self.N_LANDMARKS:
            extra_joints = torch.index_select(
                ...
                torch.tensor(
                    list(self.MANO_FINGERTIP_VERT_INDICES.values()),
                    dtype=torch.long,
                ).to(left_mano_output.vertices.device),
            )
            joints = torch.cat([left_mano_output.joints, extra_joints], dim=1)
        else:
            joints = left_mano_output.joints
    ...
    def expand_pose(self, pca_pose: torch.Tensor, handedness: str) -> torch.Tensor:
        """
        Expand 15D PCA pose into full 45D pose using MANO PCA basis.

        Args:
            pca_pose: (15,) PCA pose vector
            handedness: 'left' or 'right'

        Returns:
            (48,) full pose vector = [global_rot (3), full_joint_pose (45)]
        """
        if handedness == "right":
            mano_layer = self.mano_layer_right
        elif handedness == "left":
            mano_layer = self.mano_layer_left
        else:
            raise ValueError("Invalid handedness")

        if not self.use_pose_pca:
            raise ValueError("Pose is not using PCA in current MANO model")

        full_pose = torch.matmul(pca_pose, mano_layer.hand_components)
        return torch.cat([torch.zeros(3), full_pose], dim=0)  # global_rot is 0
...
def loadManoHandModel(
    ...
    return MANOHandModel(device, mano_model_files_dir)
